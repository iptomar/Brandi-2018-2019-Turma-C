...
exports.GET_USER_AND_TYPE_BY_EMAIL = "SELECT a.id,a.email, a.password,a.salt,a.full_name,a.address,DATE_FORMAT(a.birthday, '%d/%m/%Y') as birthday,a.cellphone,a.id_type_user,b.type_user,a.register_date,a.title,a.qualifications,a.last_login FROM " + infoDB.TBL_USERS + " a, " + infoDB.TBL_USER_TYPES + " b WHERE a.id_type_user=b.id AND email=? AND a.deleted = 0 LIMIT 1";
...
exports.GET_USER_AND_TYPE_LIST = "SELECT a.id,a.email, a.password,a.salt,a.full_name,a.address,DATE_FORMAT(a.birthday, '%d/%m/%Y') as birthday,a.cellphone,a.id_type_user,a.register_date,a.title,a.qualifications,b.type_user,a.last_login FROM " + infoDB.TBL_USERS + " a, " + infoDB.TBL_USER_TYPES + " b WHERE a.id_type_user=b.id AND a.deleted = 0 AND (a.full_name like ? OR a.email like ? OR b.type_user LIKE ?)";
...
exports.UPDATE_USER = "UPDATE " + infoDB.TBL_USERS + " SET full_name=?, address=?, cellphone=?, id_type_user=?,title=?, qualifications=? WHERE id=?";
